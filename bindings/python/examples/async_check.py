"""This is an example of running a simple check in an async environment.

This check cannot be automatically fixed. The only methods that are required
are the ``title`` (the human readable title of the check), ``description``
(human readable description of the check), and ``async_check`` (the method that
validates the input).
...
class IsEvenCheck(openchecks.AsyncBaseCheck):
...
async def main() -> None:
    ...
    result_pass = await openchecks.async_run(check_pass)

    assert not result_pass.status().has_failed()

    check_fail = IsEvenCheck(1)
    result_fail = await openchecks.async_run(check_fail)
