use base_checks::{AsyncCheck, Check, CheckMetadata};
...
impl CheckMetadata for CheckWrapper {
    ...
    fn hint(&self) -> base_checks::CheckHint {
        Python::with_gil(|py| {
            self.check
...
impl Check for CheckWrapper {
    ...
    fn check(&self) -> base_checks::CheckResult<Self::Item, Self::Items> {
        let result = Python::with_gil(|py| {
            ...
            Ok::<base_checks::CheckResult<ItemWrapper, Vec<ItemWrapper>>, PyErr>(
                base_checks::CheckResult::new(status, message, items, can_fix, can_skip, None),
            )
        });
        match result {
            Ok(result) => result,
            Err(err) => base_checks::CheckResult::new(
                base_checks::Status::SystemError,
                err.to_string(),
                None,
                false,
                false,
                Some(base_checks::Error::new(&err.to_string())),
            ),
        }
    }

    fn auto_fix(&mut self) -> Result<(), base_checks::Error> {
        Python::with_gil(|py| {
            self.check.call_method0(py, intern!(py, "auto_fix"))?;
            Ok(())
        })
        .map_err(|err: PyErr| base_checks::Error::new(&err.to_string()))
    }
}
...
impl AsyncCheck for AsyncCheckWrapper {
    ...
    async fn async_check(&self) -> base_checks::CheckResult<Self::Item, Self::Items> {
        let result = match Python::with_gil(|py| {
            ...
            Err(err) => {
                return base_checks::CheckResult::new(
                    base_checks::Status::SystemError,
                    err.to_string(),
                    None,
                    false,
    ...
    async fn async_auto_fix(&mut self) -> Result<(), base_checks::Error> {
        Python::with_gil(|py| {
            pyo3_asyncio::tokio::into_future(
                self.check
